...
import 'package:gromartconsumer/ui/payment/PaymentScreen.dart';
...
class _DeliveryAddressScreenState extends State<DeliveryAddressScreen> {
                ...
                Card(
                  ...
                  child: Column(
                    children: [
                      ...
                      Visibility(
                        child: Text("yourNewDeliveryChargeWillBe".tr() + " $symbol$deliveryCharges",
                            style: TextStyle(
  ...
  getDeliveyData() async {
    ...
    if (!widget.take_away!) {
      ...
      num km = num.parse(getKm(
          Position.fromMap({'latitude': lat, 'longitude': long}), Position.fromMap({'latitude': vendorModel!.latitude, 'longitude': vendorModel!.longitude})));
  ...
  validateForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // } else
      {
        showProgress(context, 'savingAddress'.tr(), false);
          ...
          latitude: lat == null
              ? MyAppState.currentUser!.shippingAddress.location.latitude == 0.01
                  ? showDialog(
                      ...
                      builder: (_) {
                        return AlertDialog(
                          content: Text('pleaseSelectCurrentAddressUsingGPSLocationMovePinToExactLocation'.tr()),
                          actions: [
                            // FlatButton(
              ...
              ? MyAppState.currentUser!.shippingAddress.location.longitude == 0.01
                  ? showDialog(
                      ...
                      builder: (_) {
                        return AlertDialog(
                          content: Text(
                            'pleaseSelectCurrentAddressUsingGPSLocationMovePinToExactLocation'.tr(),
                          ),
                          actions: [
                            // FlatButton(
                            //   onPressed: () => Navigator.pop(
